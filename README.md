...
| [`examples/uclibc-ng/`](examples/uclibc-ng/) | **A C library measured by its own test suite.** uClibc-ng 1.0.55 for microblazeel, 115 upstream tests under qemu-user + the plugin, merged by source line into a library-only LCOV report (40.5% lines / 54.2% functions / 30.9% branches). No `--coverage` rebuild; shows the static-link per-binary-symbolize-then-merge pattern. |
