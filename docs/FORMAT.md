...
* `count` is the number of executions attributed to that address. In
  `tb-insn` mode every instruction of a TB inherits that TB's execution count.
  A `count` of `0` should not occur (only executed blocks are emitted) but is
  not structurally illegal.
...
* Edges cross **all** transfer kinds — direct branches, indirect branches,
  calls, returns, and exception/interrupt entry — because they are derived
  from observed block sequencing, not from decoding the branch. A reader doing
  branch coverage should expect call/return and trap edges alongside
  conditional-branch edges and filter by symbol/line if it wants only the
  latter.
...
Schema (all keys are always present):

| Key                | JSON type | Meaning |
|--------------------|-----------|---------|
| `format`           | string    | Always `"tcgcov"`. |
| `version`          | number    | Always `1`. Mirrors `header.version`. |
| `mode`             | string    | `"tb"` or `"tb-insn"`. Corresponds to `record_type` `1` and `2` respectively. |
...
A real 441-byte file: `record_type = 1` (TB addresses), counts off, edges on
without edge counts, three address records and two edges, no filters.

```
metadata_size  = 297
records_offset = 88 + 297  = 385   (0x181)
records_size   = 3 * 8     = 24    (0x18)
edges_offset   = 385 + 24  = 409   (0x199)
edges_size     = 2 * 16    = 32    (0x20)
total          = 441
```

Note that `records_offset = 385` is not 8-byte aligned — as warned in §1.

```
00000000: 5443 4743 4f56 3100 0100 0100 5800 0000  TCGCOV1.....X...
00000010: 0100 0000 0200 0000 0300 0000 0000 0000  ................
00000020: 5800 0000 0000 0000 2901 0000 0000 0000  X.......).......
00000030: 8101 0000 0000 0000 1800 0000 0000 0000  ................
00000040: 0200 0000 0000 0000 9901 0000 0000 0000  ................
...
00000170: 2022 6669 6c74 6572 7322 3a20 5b5d 0a7d   "filters": [].}
00000180: 0a00 1000 0000 0000 0010 1000 0000 0000  ................
00000190: 0020 1000 0000 0000 000c 1000 0000 0000  . ..............
000001a0: 0010 1000 0000 0000 001c 1000 0000 0000  ................
000001b0: 0000 1000 0000 0000 00                   .........
```
...
| 0x28   | `29 01 ...`               | `metadata_size`   | 297 |
| 0x30   | `81 01 ...`               | `records_offset`  | 385 |
| 0x38   | `18 00 ...`               | `records_size`    | 24 |
| 0x40   | `02 00 ...`               | `edge_count`      | 2 |
| 0x48   | `99 01 ...`               | `edges_offset`    | 409 |
| 0x50   | `20 00 ...`               | `edges_size`      | 32 |

### Metadata (offsets 0x58 .. 0x180 inclusive, 297 bytes)
...
### Address records (offset 385 = 0x181, 3 × 8 bytes)

| Offset | Bytes                     | `addr`   |
|-------:|---------------------------|----------|
| 0x181  | `00 10 00 00 00 00 00 00` | `0x1000` |
| 0x189  | `10 10 00 00 00 00 00 00` | `0x1010` |
| 0x191  | `20 10 00 00 00 00 00 00` | `0x1020` |

Sorted ascending, as required.

### Edge records (offset 409 = 0x199, 2 × 16 bytes)

| Offset | `src`    | `dst`    | Reading |
|-------:|----------|----------|---------|
| 0x199  | `0x100c` | `0x1010` | the block starting at `0x1000` ends at `0x100c` and fell through / branched to the block at `0x1010` |
| 0x1A9  | `0x101c` | `0x1000` | the block starting at `0x1010` ends at `0x101c` and transferred back to `0x1000` (a loop back-edge) |
...
| `mode=tb`      | —              | `record_type = 1`, one record per translation-block start. |
| `mode=tb-insn` | *(default)*    | `record_type = 2`, one record per instruction. `mode=insn` is an accepted alias. |
...
With `edges=0` (the default) the address records are unchanged from earlier
releases; the file grows only by the 24 zero bytes of edge header fields and
the two new metadata keys, and no per-translation-block edge-tracking cost is
paid at run time.
